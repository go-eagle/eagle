...
var CmdServer = &cobra.Command{
	...
	Long:  "Generate the proto Server implementations. eg: eagle proto server api/xxx.proto -target-dir=internal/service",
	Run:   run,
}
...
func run(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Please specify the proto file. eg: eagle proto server api/xxx.proto")
		return
	}
	...
	for _, s := range res {
		serviceName := strings.Replace(strings.ToLower(s.Service), "service", "")
